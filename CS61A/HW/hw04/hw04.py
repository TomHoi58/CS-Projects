...
def prune_leaves(t, vals):
    ...
    if is_leaf(t) and label(t) in vals:
       ...
       return tree(label(t),[i for i in [prune_leaves(j,vals) for j in branches(t)] if i != None])
...
def balanced(m):
    ...
    def find_weight(m):
        sum = 0
        for i in [end(left(m)),end(right(m))]:
            if is_mobile(i):
                sum += find_weight(i)
            else:
                sum += size(i)
    ...
    for i in (left(m),right(m)):
        if is_mobile(end(i)):
            list += [length(i)*find_weight(end(i))]
            if not balanced(end(i)):
                return False
        else:
            list += [length(i)*size(end(i))]
...
def totals_tree(m):
    ...
    if is_weight(m):
        ...
        return tree(find_weight(m),[totals_tree(i) for i in [end(left(m)),end(right(m))] ])
