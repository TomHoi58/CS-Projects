...
def make_to_string(front, mid, back, empty_repr):
    ...
    def helper(lst):
        if lst is Link.empty:
            ...
            return front + str(lst.first) + mid + helper(lst.rest) + back
    return helper
...
def long_paths(tree, n):
    ...
    if tree.is_leaf() and n <= 0:
        ...
        return [Link(tree.label,j) for i in tree.branches for j in long_paths(i,n-1)]
...
def is_nested(v,L):
    if v is in L:
        ...
        return any([is_nested(v,i) for i in L])
