    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                # Revised: Modifying method can_contain doesn't work in my code, so I just keep these 
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        for _ in range(self.min_range):
            p = p.entrance
            n += 1
        while not random_or_none(p.bees) and n < self.max_range:
            n += 1
            p = p.entrance
            if p == hive:
                return None
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.True_queen:
            ...
            while p.exit:
                if p.exit.ant:
                    if p.exit.ant.is_container:
                        if p.exit.ant.is_doubled:
                            if p.exit.ant.contained_ant:
                                if not p.exit.ant.contained_ant.is_doubled:
                                    p.exit.ant.contained_ant.damage *= 2
                                    p.exit.ant.contained_ant.is_doubled = True
                        else:
                            p.exit.ant.damage *= 2
                            p.exit.ant.is_doubled = True
                            if p.exit.ant.contained_ant:
                                if not p.exit.ant.contained_ant.is_doubled:
                                    p.exit.ant.contained_ant.damage *= 2
                                    p.exit.ant.contained_ant.is_doubled = True
                    else:
                        if not p.exit.ant.is_doubled:
                            p.exit.ant.damage *= 2
                            p.exit.ant.is_doubled = True
                p = p.exit
...
def make_scare(action, bee):
    ...
    "*** YOUR CODE HERE ***"
    def new_action(colony):
        if bee.direction == "Forward" and not bee.is_scared:
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    stored_action = bee.action
    def helper(colony):
        nonlocal duration
        if duration == 0:
            bee.direction = "Forward"
            stored_action(colony)
        else:
            duration -= 1
            effect(stored_action,bee)(colony)
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if target:
            target.action = apply_effect(make_scare, target, 2)
        # END Problem EC
