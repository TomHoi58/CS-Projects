...
page_fault (struct intr_frame *f)
{
  ...
  if (!user && t->in_syscall && is_user_vaddr (fault_addr)){
    //Retrieve the user program esp rather than f->esp from syscall
    esp= t->user_esp;
  }
  ...
  bool success = false;
  void * new_page_base; 
  //Simplest case when faulting address >= esp and 2nd Case when faulting address <= esp - 32 for push (4 bytes below ) and pusha (16 bytes below)
  if (fault_addr >= esp -32){
    new_page_base = pg_round_down(fault_addr);
  }else {
    syscall_exit (-1);
  }
  ...
  if (kpage != NULL){
    success = (pagedir_get_page (t->pagedir, new_page_base) == NULL
      && pagedir_set_page (t->pagedir, new_page_base, kpage, true));
    if (success){
      return;
    }
    else{
    ...
    //Unable to allocate more pages, kill the process
    syscall_exit (-1);
  }
