    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        ghostsPos = [x.argMax() for x in livingGhostPositionDistributions]
        closestGhostPos = None
        closestDis = float("inf")
        for p in ghostsPos:
            dis = self.distancer.getDistance(pacmanPosition, p)
            if dis < closestDis:
                closestDis = dis
                closestGhostPos = p
        ...
        for a in legal:
            ...
            dis = self.distancer.getDistance(closestGhostPos, successorPosition)
            if dis < closestDis:
                closestDis = dis
                act = a
