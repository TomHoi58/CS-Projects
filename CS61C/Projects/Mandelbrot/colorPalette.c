...
int P3colorpalette(char* colorfile, int width, int heightpercolor, char* outputfile)
{
	...
	if (p == NULL || width < 1 || heightpercolor <1){
		free(colorcount);
		free(p);
		return 1;
	}
	FILE* fptr = fopen(outputfile,"w");
	fprintf(fptr,"%c%d %d %d %d\n",'P',3,width,heightpercolor * (*colorcount),255);
	// fprintf(fptr,"%u %u %u ",p[0][0],p[0][1],p[0][2]);
	// fprintf(fptr,"%u %u %u ",p[1][0],p[1][1],p[1][2]);
	// fprintf(fptr,"%u %u %u ",p[2][0],p[2][1],p[2][2]);

	for (int i = 0; i < *colorcount;i++){
		for (int h = 0; h <heightpercolor; h++){
			for (int w = 0; w<width;w++){
			if (w == width -1){
				fprintf(fptr,"%u %u %u",p[i][0],p[i][1],p[i][2]);
			}else {
				fprintf(fptr,"%u %u %u ",p[i][0],p[i][1],p[i][2]);
			}
			}
...
int P6colorpalette(char* colorfile, int width, int heightpercolor, char* outputfile)
{
	...
	FILE* fptr = fopen(outputfile,"w");
	fprintf(fptr,"%c%d %d %d %d\n",'P',6,width,heightpercolor * (*colorcount),255);
	// fprintf(fptr,"%u %u %u ",p[0][0],p[0][1],p[0][2]);
	// fprintf(fptr,"%u %u %u ",p[1][0],p[1][1],p[1][2]);
	// fprintf(fptr,"%u %u %u ",p[2][0],p[2][1],p[2][2]);
