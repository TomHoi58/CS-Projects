...
public class QueryPlan {
    ...
    public Iterator<Record> execute() {
        ...
        Map<Set, QueryOperator> intermediateMap;

        allTableNames.add(startTableName);
        allTableNames.addAll(joinTableNames);
        for (String s: allTableNames){
            Set<String> pass1Set = new HashSet<>();
            pass1Set.add(s);
            pass1Map.put(pass1Set,minCostSingleAccess(s));
        }
        ...
        // been joined.
        intermediateMap = pass1Map;
        int numofJoins = allTableNames.size() -1;
        for (int i = 0 ; i < numofJoins ; i++){
            intermediateMap = minCostJoins(intermediateMap,pass1Map);
        }
        ...
        this.finalOperator = minCostOperator(intermediateMap);
//        this.addSelects();
        this.addGroupBy();
        this.addProjects();
    ...
    QueryOperator minCostSingleAccess(String table) {
        ...
        QueryOperator indexoper;

        for (int i: getEligibleIndexColumns(table)){
            indexoper = new IndexScanOperator(this.transaction,table,this.selectColumnNames.get(i),this.selectOperators.get(i),this.selectDataBoxes.get(i));
            if (indexoper.getIOCost() < mincost){
                mincost = indexoper.getIOCost();
                minOp = indexoper;
                selectcolumnindex = i;
                indexscanused = true;
            }
        ...
        if (indexscanused){
            minOp = addEligibleSelections(minOp,selectcolumnindex);
        }else {
    ...
    Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
        ...
        boolean set_in_left_relation;
        Set<String> temp = new HashSet<>();

        for (Set<String> s: prevMap.keySet()){
            oldop = prevMap.get(s);
            for (int i = 0; i < this.joinTableNames.size(); i++){
                left_table_name = getJoinLeftColumnNameByIndex(i)[0];
                left_column_name = getJoinLeftColumnNameByIndex(i)[1];
                right_table_name = getJoinRightColumnNameByIndex(i)[0];
                right_column_name = getJoinRightColumnNameByIndex(i)[1];

                if (s.contains(left_table_name) && !s.contains(right_table_name)){
                    temp.clear();
                    temp.add(right_table_name);
                    newop = pass1Map.get(temp);
                    set_in_left_relation = true;
                    if (newop == null){
                        throw new NoSuchElementException("Pass1Map doesn't contain the table name");
                    }
                } else if(s.contains(right_table_name) && !s.contains(left_table_name)){
                    temp.clear();
                    temp.add(left_table_name);
                    newop = pass1Map.get(temp);
                    set_in_left_relation = false;
                    if (newop == null){
                        throw new NoSuchElementException("Pass1Map doesn't contain the table name");
                    }
                }else {
                ...
                Set<String> finalset = new HashSet<>(s);

                if (set_in_left_relation){
                    finalop = minCostJoinType(oldop,newop,left_column_name,right_column_name);
                    finalset.add(right_table_name);
                }else{
                    finalop = minCostJoinType(newop,oldop,left_column_name,right_column_name);
                    finalset.add(left_table_name);
                }

                if (map.containsKey(finalset)){
                    if (map.get(finalset).getIOCost() > finalop.getIOCost()){
                        map.put(finalset,finalop);
                    }
                }else{
                    map.put(finalset,finalop);
                }
