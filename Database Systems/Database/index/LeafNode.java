...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        //insert new pair to the list end
        if (i == keys.size()){
            keys.add(i,key);
            rids.add(i,rid);
        }
        ...
        if (keys.size() < metadata.getOrder()*2 +1){
            ...
            sync();
            return Optional.of(new Pair(newleafnode.keys.get(0),newleafnode.getPage().getPageNum()));
        }
...
//        //insert new elements into the leafnode
//        Pair<DataBox, RecordId> newbie = data.next();
//        DataBox key = newbie.getFirst();
//        RecordId rid = newbie.getSecond();
//
//        keys.add(key);
//        rids.add(rid);
//
//        //not over factor
//        if(keys.size() < Math.ceil(metadata.getOrder()*2*fillFactor)+1){
//            sync();
//            return Optional.empty();
//        }else {
//            //over factor
...
//        }
        ...
        while (data.hasNext() && keys.size() < Math.ceil(metadata.getOrder()*2*fillFactor)+1) {
            Pair<DataBox, RecordId> newbie = data.next();
            DataBox key = newbie.getFirst();
            RecordId rid = newbie.getSecond();
            keys.add(key);
