...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> nodefromroot = root.put(key,rid);
        if (nodefromroot.isPresent()){
            List<DataBox> newkeys = new ArrayList<>();
            List<Long> newchildren = new ArrayList<>();
            newkeys.add(nodefromroot.get().getFirst());
            newchildren.add(root.getPage().getPageNum());
            newchildren.add(nodefromroot.get().getSecond());
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        if (!(root instanceof LeafNode)){
            throw new BPlusTreeException("tree not empty before bulkloading");
        }else{
            // TODO(proj4_part2): B+ tree locking
            LockUtil.ensureSufficientLockHeld(this.lockContext,LockType.X);
            BPlusNode n = root;
            while(data.hasNext()){
                Optional<Pair<DataBox, Long>> newnodefromchild = n.bulkLoad(data,fillFactor);
                if (newnodefromchild.isPresent()){
                    List<DataBox> newkeys = new ArrayList<>();
                    List<Long> newchildren = new ArrayList<>();
                    newkeys.add(newnodefromchild.get().getFirst());
                    newchildren.add(n.getPage().getPageNum());
                    newchildren.add(newnodefromchild.get().getSecond());
                    n = new InnerNode(metadata,bufferManager, newkeys, newchildren,lockContext);
                }
            }
            updateRoot(n);
        }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        BPlusTreeIterator(){
            ...
            curiterator = curLeaf.scanAll();
        }

        BPlusTreeIterator(DataBox key){
            curLeaf = root.get(key);
            curiterator = curLeaf.scanGreaterEqual(key);
        ...
        public RecordId next() {
            if (hasNext()){
                RecordId result =  curiterator.next();
