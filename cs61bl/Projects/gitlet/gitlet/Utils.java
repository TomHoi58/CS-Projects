...
class Utils {
    ...
    //path is the whole path including the filename
...
//            System.out.println("Internal error deserializing commit.");
            obj = null;
        }
        return obj;
    ...
    // serialize the object and store it to the given path with its shaid as its filename
    // path is where you wanna store it exluding the filename
    static void serialize(Object object, String path) {
        ...
        try {
            ...
            shaid = Utils.sha1(bytes);
        } catch (IOException excp) {
            shaid = null;
            System.out.println("Internal error sha1 commit.");
        }

        //serialize the object and store

        File file = new File(path + "/" + shaid);
        try {
    ...
    static String getsha1(Object object) {
        String shaid;
        try {
            ByteArrayOutputStream stream = new ByteArrayOutputStream();
            ObjectOutputStream objectStream = new ObjectOutputStream(stream);
